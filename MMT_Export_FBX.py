...
def A_UE4_Scale(action, float, use_vis_loc, armature):
    bpy.ops.object.mode_set(mode='POSE')    
    # if we want to be keying visual location... (EXPERIMENTAL)
    if use_vis_loc:
        loc_type = 'BUILTIN_KSI_VisualLoc'
    else:
        loc_type = 'Location'
    loc_keys = {key.co[0] : [] for fcurve in action.fcurves for key in fcurve.keyframe_points}
    ...
    for curve in [fcurve for fcurve in action.fcurves if fcurve.data_path.endswith('location')]:
        ...
        for key in curve.keyframe_points:
            ...
            if curve.data_path[12:-11] in armature.pose.bones:
                ...
                # if the bone the curve is from is not already in the {keyframe : bone name} dictionary...
                if curve.data_path[12:-11] not in loc_keys[key.co[0]]:
                    # append it...
                    loc_keys[key.co[0]].append(curve.data_path[12:-11])
    # iterate through keyframe dictionary selecting all the bones that need animation curves fixing...
    for key, value in loc_keys.items():
        ...
        if len(value) > 0:
            ...
            # set the frame to fix keys for... (switching frame does not undo previous key.co changes)
            bpy.context.scene.frame_set(key)
            # keyframe the location change to fix animation curve... (hoping to find a better method than this but it works for now)
            bpy.ops.anim.keyframe_insert_menu(type=loc_type)
            # potential alternative without using an operator... | armature.pose.bones[bone].keyframe_insert(data_path="location", frame=key) | does not fix curve though... (test options={} arg)
    # back to object mode...
...
def A_Prep(action):
    ...
    if bpy.context.scene.frame_current > action.frame_range[0] and bpy.context.scene.frame_current < action.frame_range[1]:
    ...
    if MMT.E_batch_animations:
        ...
        for action in bpy.data.actions:
            ...
            if any(name in fcurve.data_path for fcurve in action.fcurves for name in bone_names):
                ...
                A_UE4_Scale(action, scale, MMT.E_use_visual_location, armature)
                # export...
